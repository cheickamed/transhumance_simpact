/**
* Name: reseausocialgrille
* Based on the internal empty template. 
* Author: Cheick Amed Diloma Gabriel TRAORE
* Tags: 
*/
model reseausocialgrille
...
global {
	...
	int nbr_elmt_res_soc <- 10 min: 5 max: 11;
	int jour_res_soc <- 5 min: 0 max: 7; //parameter: 'Nb jour' category: 'Reseau_social';
	float d_res_soc <- 100 #km min: 75 #km max: 150 #km parameter: 'Dist_soc' category: 'Reseau_social';
...
//create roads from: shape_file_roads with: [sous_type::string(get("SOUSTYP"))];
	...
	reflex save_d_rs when: is_batch_analyse and cpt_trp_retour >= nb_trp - 1 {
		save [nb_cycle_aller, moy_cycle_aller, d_res_soc / 1000, jour_res_soc, nbr_elmt_res_soc, propor_utilisa_aller] rewrite: false to: "dist_rs_entier_aller.csv" type: csv;
		save [nb_cycle_retour, moy_cycle_retour, d_res_soc / 1000, jour_res_soc, nbr_elmt_res_soc, propor_utilisa_retour] rewrite: false to: "dist_rs_entier_retour.csv" type: csv;
	}
...
grid grille width: 36 height: 38 neighbors: 8 {
	...
	reflex densite_aller when: cpt_trp_aller >= nb_trp - 1 and enregistrement and is_batch {
		save [grid_x, grid_y, nb_trp_inside, s_nb_trp_inside, d_res_soc / 1000, jour_res_soc, nbr_elmt_res_soc] to: 'res_soc_aller_50r.csv' rewrite: false type: 'csv';
		nb_trp_inside <- 0;
		s_nb_trp_inside <- 0;
	...
	reflex densite_retour when: cpt_trp_retour >= nb_trp - 1 and enregistrement2 and is_batch {
		save [grid_x, grid_y, nb_trp_inside, s_nb_trp_inside, d_res_soc / 1000, jour_res_soc, nbr_elmt_res_soc] to: 'res_soc_retour_50r.csv' rewrite: false type: 'csv';
		enregistrement2 <- !enregistrement2;
	}

}

experiment res_soc_espace_entier type: gui {
...
experiment sauv_donne type: batch repeat: 50 until: cpt_trp_retour >= nb_trp {
	...
	reflex save_rs {
		...
		(each.moy_cycle_aller)) / length(simulations), standard_deviation(simulations collect
		(each.moy_cycle_aller)), simulation.d_res_soc / 1000, jour_res_soc, nbr_elmt_res_soc, sum(simulations collect
		(each.propor_utilisa_aller)) / length(simulations), standard_deviation(simulations collect (each.propor_utilisa_aller))] rewrite: false to: "cycle_rs_espace_entier_aller.csv"
		type: csv;
		...
		(each.moy_cycle_retour)) / length(simulations), standard_deviation(simulations collect
		(each.moy_cycle_retour)), simulation.d_res_soc / 1000, jour_res_soc, nbr_elmt_res_soc, sum(simulations collect
		(each.propor_utilisa_retour)) / length(simulations), standard_deviation(simulations collect (each.propor_utilisa_retour))] rewrite: false to: "cycle_rs_espace_entier_retour.csv"
		type: csv;
	}
...
experiment analyse_para_rs type: batch repeat: 4 keep_seed: true until: cpt_trp_retour >= nb_trp {
