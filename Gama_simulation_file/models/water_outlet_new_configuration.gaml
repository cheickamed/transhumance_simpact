...
global {
	file shape_file_forage <- file("../includes/forage_creer.shp");
...
//create roads from: shape_file_roads with: [sous_type::string(get("SOUSTYP"))];
	...
	reflex save_deau when: is_batch_analyse and cpt_trp_retour >= nb_trp - 1 {
		save [nb_cycle_aller, moy_cycle_aller, d_eau / 1000, propor_utilisa_aller] rewrite: false to: "dist_forage_creer_espace_entier_aller.csv" type: csv;
		save [nb_cycle_retour, moy_cycle_retour, d_eau / 1000, propor_utilisa_retour] rewrite: false to: "dist_forage_creer_espace_entier_retour.csv" type: csv;
	}
...
grid grille width: 36 height: 38 neighbors: 8 {
	...
	reflex densite_aller when: cpt_trp_aller = nb_trp and enregistrement and is_batch {
		save [grid_x, grid_y, nb_trp_inside, s_nb_trp_inside, impact_trp_aller] to: 'forage_aller_creer_espace_entier_50r.csv' rewrite: false type: 'csv';
		nb_trp_inside <- 0;
		s_nb_trp_inside <- 0;
	...
	reflex densite_retour when: cpt_trp_retour >= nb_trp - 1 and enregistrement2 and is_batch {
		save [grid_x, grid_y, nb_trp_inside, s_nb_trp_inside, impact_trp_retour] to: 'forage_retour_creer_espace_entier_50r.csv' rewrite: false type: 'csv';
		enregistrement2 <- !enregistrement2;
	}

}

experiment forage_espace_creer type: gui {
...
experiment sauv_donne type: batch repeat: 50 until: cpt_trp_retour >= nb_trp {
	...
	reflex save {
		...
		"cycle_forage_creer_espace_entier_aller.csv" type: csv;
		...
		"cycle_forage_creer_espace_entier_retour.csv" type: csv;
	}
