...
* Author: Cheick Amed Diloma Gabriel
* Tags: 
*/
model vegetation
...
//initialisation de plvt
	...
	reflex update_comptage_trp when: every(step) {
	...
	reflex save_dveg when: is_batch_analyse and (cpt_trp_retour >= nb_trp or cycle >= 479) {
		save [nb_cycle_aller, moy_cycle_aller, d_veg / 1000, propor_utilisa_aller] rewrite: false to: "dist_vegetation_espace_entier_aller.csv" type: csv;
		save [nb_cycle_retour, moy_cycle_retour, d_veg / 1000, propor_utilisa_retour] rewrite: false to: "dist_vegetation_espace_entier_retour.csv" type: csv;
	} }
...
species troupeau skills: [moving] {
	...
	//-------------------------------- déplacement du trp -------------------------------------
	reflex trp_vegetation when: (self.objectif != 'vegetation' and self.objectif != 'quitter_evitement') or objectif = 'deplacement' {
		if phase = 'aller' {
			if my_cell.voisins != [] {
				ask my_cell.voisins {
					...
					self.location.y <= myself.terr_acc.y) {
						...
						myself.position_vegetation_visite_aller <- myself.position_vegetation_visite_aller + [self.location];
			...
			if my_cell.voisins != [] {
				ask my_cell.voisins {
					...
					self.location.y >= myself.terr_orig.y) {
						...
						myself.position_vegetation_visite_retour <- myself.position_vegetation_visite_retour + [self.location];
	...
	reflex trp_a_vegetation when: objectif = 'vegetation' or objectif = 'quitter_evitement' {
	...
	reflex evitement when: objectif = 'evitement' {
		ask my_cell neighbors_at 1 where (each.color != #red) {
			myself.my_cell.location <- one_of(self.location);
			myself.objectif <- 'deplacement';
			/*if myself.phase = 'aller' {
			myself.position_vegetation_visite_aller <- myself.position_vegetation_visite_aller + [self.location];
			} else {
			myself.position_vegetation_visite_retour <- myself.position_vegetation_visite_retour + [self.location]; // evite que le troupeau parte deux fois au même forage
			}*/
		}
	...
	//permet de réorienter le trp vers le terroir cible
		if cpt_trp_aller >= nb_trp and test_retour {
			phase <- 'retour';
			the_target <- terr_orig;
...
grid grille width: 36 height: 38 neighbors: 8 {
	...
	reflex densite_aller when: cpt_trp_aller = nb_trp and enregistrement and is_batch {
		save [grid_x, grid_y, nb_trp_inside, s_nb_trp_inside] to: 'vegetation_aller_50r.csv' rewrite: false type: 'csv';
		nb_trp_inside <- 0;
		s_nb_trp_inside <- 0;
	...
	reflex densite_retour when: cpt_trp_retour >= nb_trp - 1 and enregistrement2 and is_batch {
		save [grid_x, grid_y, nb_trp_inside, s_nb_trp_inside] to: 'vegetation_retour_50r.csv' rewrite: false type: 'csv';
		enregistrement2 <- !enregistrement2;
	}

}

experiment vegetation_espace_entier type: gui {
	output {
	//layout #split;
		monitor "Pluviométrie " value: plvt refresh: every(1 #month);
		display mvt_grille type: java2D {
		//species vegetation aspect: asp_vegetation_base;
			grid grille border: #black; // triangulation: true; // ;
			species zone aspect: asp_zone transparency: 0.3 refresh: false;
			species troupeau aspect: asp_trp;
		}

		display graf type: java2D refresh: every(1 #week) {
...
//******************************* batch *************************
experiment sauv_donne type: batch repeat: 50 until: cpt_trp_retour >= nb_trp or cycle >= 480 { //
	...
	reflex save {
		...
		(each.moy_cycle_aller)) / length(simulations), standard_deviation(simulations collect (each.moy_cycle_aller)), simulation.d_veg / 1000, sum(simulations collect
		(each.propor_utilisa_aller)) / length(simulations), standard_deviation(simulations collect (each.propor_utilisa_aller))] rewrite: false to: "vegetation_espace_entier_aller.csv"
		type: csv;
		...
		(each.moy_cycle_retour)) / length(simulations), standard_deviation(simulations collect (each.moy_cycle_retour)), simulation.d_veg / 1000, sum(simulations collect
		...
		"vegetation_espace_entier_retour.csv" type: csv;
	}

}

experiment analyse_dist_vegetation type: batch repeat: 20 keep_seed: true until: cpt_trp_retour >= nb_trp or cycle >= 480 { //
